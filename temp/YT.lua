...
-- https://github.com/grafi-tt/lunajson/blob/master/src/lunajson/decoder.lua
	...
	local function ShowDescInfo()
		...
		t.showTime = 10000
		t.once = true
		t.message = desc_html(m_simpleTV.User.YT.desc,
			string.format('https://i.ytimg.com/vi/%s/default.jpg', m_simpleTV.User.YT.vId),
	...
	local function ItagTab()
		local video = {
						...
						697, 397, 333, 244, 135, 168, -- 480
						698, 398, 334, 302, 247, 298, 136, 169, 22, -- 720
						699, 399, 335, 303, 248, 616, 299, 137, 216, 170, -- 1080
						700, 400, 336, 308, 271, 304, 264, -- 1440
	...
	local function Stream_Live(hls, title)
		...
		local t = {}
			for w in answer:gmatch('EXT%-X%-STREAM%-INF(.-)\n') do
				local name = tonumber(w:match('RESOLUTION=%d+x(%d+)') or '0')
	...
	local function GetVideoInfoAndroid()
		local session_videoInfo = m_simpleTV.Http.New('com.google.android.youtube/20.10.38 (Linux; U; Android 11) gzip')
			if not session_videoInfo then return end
		m_simpleTV.Http.SetTimeout(session_videoInfo, 16000)
		local clientName = 'ANDROID'
		local clientVersion = '20.10.38'
		local thirdParty = urlAdr:match('$OPT:http%-referrer=([^%$]+)') or 'https://www.youtube.com/'
	...
	local function GetStreamsTab(vId)
		...
		if tab.streamingData then
			...
			if tab.streamingData.adaptiveFormats1 then
				local k = 1
	...
	function SavePlst_YT()
		if m_simpleTV.User.YT.Plst and m_simpleTV.User.YT.plstHeader then
			...
			if fhandle then
				...
				fhandle:close()
				t.icon = 'NoIcon'
				t.message = m_simpleTV.User.YT.Lng.savePlst_1 .. ':\n'
				...
				t.icon = 'Critical'
				t.message = m_simpleTV.User.YT.Lng.savePlst_3
			end
			m_simpleTV.Interface.MessageBoxT(t)
		end
	end
	function Qlty_YT()
