-- видеоскрипт для сайта https://www.youtube.com (29/7/22)
...
-- https://github.com/videolan/vlc/blob/master/share/lua/playlist/youtube.lua
		...
		datac = datac .. ','
			while datac ~= '' do
				...
				elseif string.match(datac, '^-?%d+,') or string.match(datac, '^%-?%d+[eE]%-?%d+,') then
					el, datac = string.match(datac, '^(.-),(.*)$')
					el = tonumber(el)
	...
	local function GetVideoInfo(clientName, clientVersion)
		...
		clientVersion = clientVersion or '1.20220728.00.00'
